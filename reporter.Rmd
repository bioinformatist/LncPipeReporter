...
knitr::opts_chunk$set(
  ...
  exprs <- data.table(features, t(scale(t(exprs)))[col.order, row.order])
...
py <- ggdend(dy$segments) + coord_flip()  
  ...
  heatmap.exprs <- ggplot(exprs, aes(x = sample.name, y = features)) + geom_tile(aes(fill=value)) + scale_fill_gradient2(low = "green", mid = "black", high = "red", midpoint = 0)
...
p_empty <- ggplotly(ggplot()) %>%
  # note that margin applies to entire plot, so we can
  # add it here to make tick labels more readable
  layout(margin = list(l = 200),
         xaxis = eaxis,
         yaxis = eaxis)
  # return(list(dendrogram.x, ggplot(), heatmap.exprs, dendrogram.y))
subplot(px, p_empty, heatmap.exprs, py, nrows = 2, margin = 0.01)
}
```
...
ggheatmap(heatmap.m)
plot_grid(plotlist = heatmap.dendrogram, align = 'hv')
```
