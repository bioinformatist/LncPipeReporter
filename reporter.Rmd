...
#read from N163.log
hisat <- fread(
"cat N1037.log |perl -F'\\(|\\)|(>?\\d\\stimes?)' -lanE '/%/ and /aligned/ ? say join qq{\t},@F : next'")

#modify table
hisat <- hisat[,c(1,3,5,6)][, ':=' (V3 =  as.numeric(sub("%", "", V3, fixed = TRUE)) / 100)][,':='(V5 = sub(' exactly',"",V5))]
setnames(hisat, c('Num','Percentage','Type', 'Times'))
```
