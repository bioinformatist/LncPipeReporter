...
options(DT.options = list(autoFill = TRUE, searchHighlight = TRUE, search = list(regex = TRUE, caseInsensitive = FALSE), dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
...
```{r}
DT::datatable(hisat) %>% DT::formatRound('Percentage', digits = 2)
rm(hisat)
```
...
cdf[, type := .toupperFirstLetter(sub("_", " ", type))]
...
```{r}
DT::datatable(cdf) %>% DT::formatRound('CPAT', digits = 2)
```
...
gtf <-
  ...
  gtf[, Type := .toupperFirstLetter(Type)]
```
