...
log.summary <- log.summary[V2 %like% c('of i|Uni|mu|many l|sho|oth')][, V1 := tstrsplit(V1, .Platform$file.sep)[[length(tstrsplit(V1, .Platform$file.sep))]]][, V1 := tstrsplit(V1, '[._]')[[1]]]
...
if (nrow(unique(log.percentage[,1])) == 1) {
  ...
  save_plot('STAR.tiff', p, base_height = 8.5, base_width = 11, dpi = 300, compression = 'lzw')
  ggplotly(p)
}
...
```{r star table}
fwrite(log.summary, 'STAR.csv')
DT::datatable(head(log.summary, n = 80L), colnames = c('Sample', 'Type', 'Value'), extensions = 'Buttons') %>% DT::formatRound('V3', digits = 2)
```