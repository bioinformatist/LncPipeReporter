...
de$table$threshold <-
  ...
  linetype = "dashed",
  size = 1
  ) + theme(legend.position = "none") + scale_x_continuous(limits = c(-10, 10)) + get(paste0('scale_color_',params$theme))()
  ggplotly(p)
```
...
**Reference** <br>
 McCarthy DJ, Chen Y and Smyth GK (2012). Differential expression analysis of multifactor RNA-Seq experiments with respect to
...
```{r compare density}
compare.dt <- data.table::data.table(Type = as.vector(t(rna.type))[as.numeric(row.names(log.cpm))], log.cpm)
p <- ggplot() + geom_density(data = melt(compare.dt, id = 'Type'), aes(x = value, colour = Type)) + get(paste0('scale_color_',params$theme))()
ggplotly(p)
```
