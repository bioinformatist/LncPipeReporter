...
save_plot(file.path(params$figures, 'vocano.tiff'), p, base_height = 8.5, base_width = 11, dpi = 300, compression = 'lzw')
ggplotly(p)
```
...
save_plot(file.path(params$figures, 'pca.tiff'), p, base_height = 8.5, base_width = 11, dpi = 300, compression = 'lzw')
ggplotly(p)
rm(pca.m, p)
invisible(gc())
```
...
```{r}
fwrite(de$table[, -5], file.path(params$tables, 'DE.csv'), row.names = TRUE)
